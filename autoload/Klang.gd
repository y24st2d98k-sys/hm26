...
## Schiedsrichterpfeife: zwei nah beieinander liegende Töne plus Luftrauschen.
func _pfiff(dauer: float) -> PackedFloat32Array:
	var a := _puffer(dauer)
	var n := a.size()
	for i in range(n):
		var t: float = float(i) / float(RATE)
		var vib: float = sin(TAU * 34.0 * t) * 14.0
		var grund: float = 2850.0 + vib
		var wert: float = sin(TAU * grund * t) * 0.6
		wert += sin(TAU * (grund * 1.012) * t) * 0.5
		wert += sin(TAU * (grund * 2.02) * t) * 0.14
		wert += _rng.randf_range(-1.0, 1.0) * 0.09
		a[i] = wert * _huelle(i, n, 0.012, 0.09)
	return _normieren(a, 0.55)

## Jubel: gefiltertes Rauschen mit Anschwellen und einem tiefen Bauch.
func _jubel(dauer: float, kraft: float) -> PackedFloat32Array:
	...
	for i in range(n):
		var t: float = float(i) / float(RATE)
		var gesamt: float = float(n) / float(RATE)
		# Schneller Anstieg, langes Ausklingen — wie eine Welle durch die Halle
		var form: float = pow(clampf(t / (gesamt * 0.18), 0.0, 1.0), 0.7)
		form *= pow(clampf((gesamt - t) / (gesamt * 0.85), 0.0, 1.0), 1.4)
		# Ein tiefer Rumpler gibt dem Jubel Körper
		var rumpel: float = sin(TAU * 62.0 * t) * 0.16 * form
		a[i] = (a[i] * 3.4 + rumpel) * form * kraft
	return _normieren(a, 0.78 * clampf(kraft + 0.25, 0.3, 1.0))

## Grundgeräusch der Halle: langsam wogendes, tiefes Rauschen als Schleife.
func _atmosphaere(dauer: float) -> PackedFloat32Array:
	...
	for i in range(n):
		var t: float = float(i) / float(RATE)
		var woge: float = 0.72 + 0.28 * sin(TAU * 0.13 * t) * sin(TAU * 0.071 * t + 1.3)
		a[i] = a[i] * 4.2 * woge
	# Nahtlose Schleife: die letzten 0,3 s in den Anfang blenden
	var blende: int = int(0.3 * float(RATE))
